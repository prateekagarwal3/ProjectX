...
N = 1454
Eps = 0.0000000001
Pz_d_w = np.zeros((Strokes,N,32,32),dtype = np.float64)
Pw_z  = np.zeros((32,32,Strokes),dtype = np.float64)
			...
			#####  E-Step  #####
				...
				for k in xrange(Strokes) :
					for i in xrange(N) :
						for x in xrange(32) :
							for y in xrange(32) :
								Pz_d[k][i] = ( Pz_d[k][i] + (float(imgPixels[x][y]) / 255.0) * Pz_d_w[k][i][x][y] ) / ( 32 * 32 )
