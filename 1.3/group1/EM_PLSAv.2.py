...
for k in xrange(Strokes) : #Initialization
	for i in xrange(N) :
		for m in range(M):
			...
			normalizesum += Pz_d_w[k][i][m]
Pz_d_w /= float (normalizesum)
normalizesum= 0
				
for m in xrange(M):
	for k in xrange(Strokes) :
		Pw_z[m][k] = random.random()
		normalizesum += Pw_z[m][k]
Pw_z /= float (normalizesum)
normalizesum= 0

for k in xrange(Strokes) :
	for i in xrange(N) :
		Pz_d[k][i] = random.random()
		normalizesum += Pz_d[k][i]
Pz_d /= float (normalizesum)
...
for k in xrange(Strokes):
	...
	img.show();
	img.save("Stroke/StrokeImagev2" + str(k) + ".tif")	
